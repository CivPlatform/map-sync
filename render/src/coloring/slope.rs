...
const EXCLUDE_IDS: [std::ops::RangeInclusive<u16>; 5] = [
    ...
    8143..=8144, // tall_grass
    9720..=9746, // kelp, kelp_plant
];
...
pub fn get_slope_color(map: &ChunkMap, x: i32, z: i32) -> u32 {
    match get_column_in_map(map, x, z) {
        None => TRANSPARENT,
        Some(_col) => {
            let y = get_slope_y(0, map, x, z);
            let mut ditch = false;
            let mut raise = false;

            // 1m away
            let dy_w1 = y - get_slope_y(y, map, x - 1, z);
            let dy_n1 = y - get_slope_y(y, map, x, z - 1);
            let dy_nw = y - get_slope_y(y, map, x - 1, z - 1);
            ...
            // 2m away
            let dy_w2 = y - get_slope_y(y, map, x - 2, z - 1);
            let dy_n2 = y - get_slope_y(y, map, x - 1, z - 2);
            let dy_nw2 = y - get_slope_y(y, map, x - 2, z - 2);
