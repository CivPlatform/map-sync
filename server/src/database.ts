...
export async function setup() {
    ...
    await get()
        ...
        .addForeignKeyConstraint("FK_chunk_ref", ["hash"], "chunk_data", ["hash"], (fk) =>
            fk.onUpdate("no action").onDelete("no action"),
        )
        .execute();
}
                ...
                .select([
                    ...
                    (eb) =>
                        kysely.sql<string>`(cast(floor(${eb.ref(
                            "chunk_x",
                        )} / 32.0) as int) || '_' || cast(floor(${eb.ref("chunk_z")} / 32.0) as int))`.as("region"),
                    "chunk_x as x",
                    "chunk_z as z",
                    (eb) => eb.fn.max("ts").as("timestamp"),
                ])
 ...
 */
export async function getChunkData(dimension: string, chunkX: number, chunkZ: number) {
    return get()
        .selectFrom("player_chunk")
 ...
 */
export async function getRegionChunks(dimension: string, regionX: number, regionZ: number) {
