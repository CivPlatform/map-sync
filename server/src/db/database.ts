...
export default class DatabaseConnection {
    public readonly internal: Kysely<{
        chunk_data: {
            hash: Buffer;
            version: number;
            data: Buffer;
    ...
    /**
     * See {@link BunSqliteDatabase}'s constructor and {@link https://bun.sh/docs/api/sqlite Bun SQLite} for
     * documentation on what this function can accept.
     */
    public constructor(
        ...args: ConstructorParameters<typeof BunSqliteDatabase>
    ) {
        this.internal = new Kysely({
            dialect: new BunSqliteDialect({
                database: new BunSqliteDatabase(...args),
            }),
        });
    }

    public getMigrations(): Migrator {
        return new Migrator({
            db: this.internal,
    ...
    public async setup() {
        const results = await this.getMigrations().migrateToLatest();
        for (const result of results.results ?? []) {
            switch (result.status) {
                case "Success":
                    console.info(
                        `Migration [${result.migrationName}] applied!`,
                    );
                    break;
                case "Error":
                    console.error(
    ...
    async getRegionTimestamps(dimension: string) {
    ...
    public async getChunkTimestamps(dimension: string, regions: Pos2D[]) {
        return await this.internal
            .selectFrom("player_chunk")
            .select([
                "chunk_x as chunkX",
     ...
     *
     * TODO: May want to consider making world, x, z, and timestamp a unique in
     *       the database table... may help performance.
     */
    ...
    public async storeChunkData(
        ...
        await this.internal.transaction().execute(async (transaction) => {
            await transaction
