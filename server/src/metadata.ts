	...
	if (url !== undefined) {
		...
		WHITELIST_MUTEX.runExclusive(async () => {
			...
			if (get_os_error(e) === OsError.FileNotFound) {
				...
				console.error(e);
				console.error(
					'[Whitelist] The whitelist will be loaded as empty'
				);
			}
		});
	}
...
export async function uuid_cache_save() {
	UUID_CACHE_MUTEX.runExclusive(async () => {
		await lib_fs.promises.writeFile(
			UUID_CACHE_FILE,
			JSON.stringify(Object.fromEntries(uuid_cache))
		);
		console.log('[UUID Cache] Saved UUID cache');
	}).catch((e) => {
		console.error(
			'[UUID Cache] Error occurred while saving the whitelist to the disk'
		);
		console.error(e);
	});
}
...
export async function uuid_cache_load() {
	UUID_CACHE_MUTEX.runExclusive(async () => {
		...
		console.error(
			'[UUID Cache] An error occurred when attempting to read `uuid_cache.json`'
		);
		console.error(e);
