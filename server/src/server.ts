...
export class TcpClient {
            ...
            try {
                ...
                while (true) {
                    ...
                    if (frameSize > this.maxFrameSize) {
                        return this.kick("Frame too large: " + frameSize + " have " + accBuf.length);
                    }
    ...
    private async sendInternal(pkt: ServerPacket, doCrypto = false) {
        if (!this.socket.writable) return this.debug("Socket closed, dropping", pkt.type);
        if (doCrypto && !this.cryptoPromise) throw new Error(`Can't encrypt: handshake not finished`);
    ...
    private async handleEncryptionResponsePacket(pkt: EncryptionResponsePacket) {
        if (this.cryptoPromise) throw new Error(`Already authenticated`);
        if (!this.claimedMojangName) throw new Error(`Encryption has not started: no mojangName`);
        if (!this.verifyToken) throw new Error(`Encryption has not started: no verifyToken`);

        const verifyToken = this.server.decrypt(pkt.verifyToken);
        if (!this.verifyToken.equals(verifyToken)) {
            throw new Error(`verifyToken mismatch: got ${verifyToken} expected ${this.verifyToken}`);
        }
...
async function fetchHasJoined(args: { username: string; shaHex: string; clientIp?: string }) {
    ...
    try {
        ...
        const uuid = id.replace(/^(........)-?(....)-?(....)-?(....)-?(............)$/, "$1-$2-$3-$4-$5");
        return { uuid, name };
    } catch (err) {
        console.error(res);
        throw err;
