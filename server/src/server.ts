...
export class TcpServer {
	...
	constructor(public readonly handler: ProtocolHandler) {
...
const KEY_PAIR: KeyPairKeyObjectResult = crypto.generateKeyPairSync('rsa', {
	modulusLength: 1024,
})
const PUBLIC_KEY_BUFFER: Buffer = KEY_PAIR.publicKey.export({
	type: 'spki',
	format: 'der',
})
...
export class TcpClient {
	...
	private async handlePacketReceived(packet: ClientPacket) {
		if (!this.mcPlayerUuid) {
			...
			switch (packet.type) {
				...
				default:
					throw new Error(
						`Packet ${packet.type} from unauthenticated client ${this.id}`,
					)
			}
		} else {
	...
	private async handleEncryptionResponsePacket(
		...
		if (!this.verifyToken.equals(verifyToken)) {
			throw new Error(
				`verifyToken mismatch: got [${verifyToken}] but expected [${this.verifyToken}]`,
			)
		}
		...
		const shaHex = crypto
			...
			.digest()
			.toString('hex')
		this.cryptoPromise = fetchHasJoined(this.claimedMojangName, shaHex).then(
			async (mojangAuth) => {
				if (!mojangAuth?.uuid) {
